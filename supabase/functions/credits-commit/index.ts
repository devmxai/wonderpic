...
function mapCommitError(message: string): number {
  if (
    message.includes('HOLD_NOT_FOUND') ||
    message.includes('HOLD_NOT_RESERVABLE') ||
    message.includes('HOLD_EXPIRED')
  ) {
    return 409;
  }
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const payload = (await req.json().catch(() => ({}))) as CommitPayload;
    ...
    const { data, error } = await supabase.rpc('app_commit_credits', {
    ...
    if (error) {
      return jsonResponse(mapCommitError(error.message), {
        ok: false,
        error: error.message,
        code: error.code,
      });
